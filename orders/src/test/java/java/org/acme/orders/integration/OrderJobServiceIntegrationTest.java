package java.org.acme.orders.integration;
...
public class OrderJobServiceIntegrationTest extends BaseIntegrationTest {
    ...
    void findByIdTest_idExists() {
        // Arrange
        // Act
        OrderJobDTO woJobDTO = service.findById(1L);
        // Assert
    ...
    void findByIdTest_idNotExists() {
        // Arrange
        // Act
        OrderJobDTO woJobDTO = service.findById(100L);
        // Assert
    ...
    void findByIdsTest_someIdExist() {
        // Arrange
        // Act
        List<OrderJobDTO> dtos = service.findByIds(List.of(1L,3L,20L,30L));
        // Assert
        ...
        Assertions.assertTrue(dtos.stream().map(OrderJobDTO::getWoNumber).toList().containsAll(List.of("ABC123","ABC456")),
    ...
    void findByIdsTest_noneIdExist() {
        // Arrange
        // Act
        List<OrderJobDTO> dtos = service.findByIds(List.of(20L, 30L,50L));
        // Assert
        Assertions.assertInstanceOf(List.class,dtos,"The method should return a list");
    ...
    void findByCodesTest_someCodesExist() {
        // Arrange
        // Act
        List<OrderJobDTO> dtos = service.findByCodes(List.of("JobCode1","none"));
        // Assert
        Assertions.assertInstanceOf(List.class,dtos,"The method should return a list");
        Assertions.assertFalse(dtos.isEmpty(), "There should be at least one object in the list");
        Assertions.assertEquals(2,dtos.size(),"There should be one object in the list");
        Assertions.assertTrue(dtos.stream().map(OrderJobDTO::getWoNumber).toList().contains("ABC123"),
    ...
    void findByCodesTest_noCodeExist() {
        // Arrange
        // Act
        List<OrderJobDTO> dtos = service.findByCodes(List.of("some-fruit","non-existing"));
        // Assert
        Assertions.assertInstanceOf(List.class,dtos,"The method should return a list");
        Assertions.assertTrue(dtos.isEmpty(), "There should be no objects in the list");
    }
