package java.org.acme.orders.api;
...
public class OrderJobControllerTest extends BaseApiTest {
    ...
    void findByCodesTest_success() throws Exception {
        // Arrange
        List<OrderJob> woJobs = woJobDAO.findByCodes(List.of("JobCode1","JobCode2"));
        List<OrderJobDTO> dtoList = woJobs.stream().map(woJobMapper::convertToDTO).toList();
        // Act
        ...
        TypeToken<List<OrderJobDTO>> typeToken = new TypeToken<>(){};
        List<OrderJobDTO> dtos = new Gson().fromJson(json, typeToken.getType());
        // Assert
