...
public interface OrderJobDAO extends JpaRepository<OrderJob,Long> {
    ...
    @Query(value = "select wj.* from wo.WO_JOB wj inner join wo.WO_ORDER o on wj.order_id = o.id where o.wo_number = ?1", nativeQuery = true)
    List<OrderJob> findByOrderNumber(String woNumber);
