package org.acme.work_order.jobtype.internal;
...
public interface JobTypeDAO extends JpaRepository<JobType,Long> {
    ...
    @Query(value="select * from wo.JOB_TYPE where code in ?1",nativeQuery = true)
    List<JobType> findByCodes(List<String> codes);
    @Query(value="select * from wo.JOB_TYPE where code in ?1 and active_status = ?2",nativeQuery = true)
